...
def _to_response(config) -> FoundationFormConfigResponse:
    # config.fields/categories are FoundationFormField/Category model instances
    # (from app.models.foundation_form_config) - a different class than the
    # FoundationFormFieldConfig/etc. response schema types, even though the
    # field names match. Pydantic v2 won't auto-coerce one model instance into
    ...
    #
    # Programs are absent by design: they're their own module now
    # (GET /programs), so this editor no longer round-trips them.
