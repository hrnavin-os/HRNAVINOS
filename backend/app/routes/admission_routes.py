...
async def create_admission(
    ...
    return AdmissionResponse.model_validate(await AdmissionService().create(payload, actor_id=actor.id))
...
async def list_admissions(
    ...
    result = await AdmissionService().list(params, student_id=student_id)
...
async def get_admission(
    ...
    return AdmissionResponse.model_validate(await AdmissionService().get(admission_id))
...
async def update_admission(
    ...
    return AdmissionResponse.model_validate(
        await AdmissionService().update(admission_id, payload, actor_id=actor.id)
    )
...
async def delete_admission(
    ...
    await AdmissionService().delete(admission_id, actor_id=actor.id)
