...
Installs Python 3.12, Node 22, MongoDB 7, Nginx, PM2, Certbot; creates the
`hrnavinos` system user and `/var/www/hrnavinos-erp`.

### 2. (Optional but recommended) Secure MongoDB

By default `setup_server.sh` leaves MongoDB with no auth, bound to
localhost only — fine to start with, but for production enable auth (see
the printed next-steps at the end of `setup_server.sh` for the exact
`mongosh` command and `/etc/mongod.conf` change).
...
# edit .env: MONGODB_URI, MONGODB_DB_NAME=hrnavinos_erp, SECRET_KEY (openssl rand -base64 48), CORS_ORIGINS, FIRST_SUPERUSER_*
```
...
Pulls `main`, installs backend deps, restarts the `hrnavinos-backend`
systemd service (Beanie creates/updates indexes on startup — no separate
migration step), builds the frontend, and (re)starts it under PM2
...
cp .env.example .env.prod   # MONGODB_DB_NAME, SECRET_KEY, CORS_ORIGINS, VITE_API_BASE_URL
docker compose -f docker/docker-compose.prod.yml --env-file .env.prod up -d --build
```

MongoDB is not exposed to the host; backend binds `127.0.0.1:8000` and
...
# Restore
bash deployment/scripts/restore.sh /var/backups/hrnavinos-erp/db_20260101_020000.archive \
                                    /var/backups/hrnavinos-erp/uploads_20260101_020000.tar.gz
```

`backup.sh` runs `mongodump` (gzip'd archive) and tars
`backend/app/uploads/`, pruning backups older than `BACKUP_RETENTION_DAYS`
(default 14). `restore.sh` prompts for confirmation before dropping the
database and restoring with `mongorestore`.

## CI

`.github/workflows/ci.yml` runs on every push/PR to `main`/`develop`:
backend lint (`ruff`) → backend tests (`pytest`, against a real MongoDB
