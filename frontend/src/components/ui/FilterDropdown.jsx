...
// The chrome every filter in the row shares: the bordered shell, the trigger
// that names the filter until something is picked, the inline clear "x", and
// the portaled menu. Split out from FilterDropdown so a filter whose menu
// isn't a list of options - the date one, with its presets and two inputs -
// can sit in the same row without a second, almost-identical implementation
...
export function FilterShell({ label, activeLabel, onClear, grow = false, menuWidth = 260, children }) {
  ...
  function toggle() {
    ...
    setMenuPosition(menuPositionFor(shellRef.current.getBoundingClientRect(), menuWidth))
  }
  ...
  return (
    <div className={grow ? 'w-full min-w-0' : 'inline-block'}>
      ...
      >
        ...
        >
          <span className="max-w-40 truncate">{isActive ? activeLabel : label}</span>
          <ChevronDown
        ...
        {isActive && (
          // Clearing one of seven filters shouldn't mean reopening its menu to
          // hunt for "All ...".
          <button
            type="button"
            onClick={onClear}
...
export function MenuOption({ label, isSelected, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
