    ...
    <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-center sm:gap-8">
      <div className="relative shrink-0">
        <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} role="img" aria-label={`${centerLabel} ${total}`}>
          ...
          <g transform={`rotate(-90 ${SIZE / 2} ${SIZE / 2})`}>
            {slices.map((slice) => {
              ...
              // A single full-circle slice has no neighbour to be separated
              // from, and taking a gap out of it leaves a nick in an
              // otherwise unbroken ring.
              ...
              return (
                ...
                >
                  {/* Hover detail, but never the only way to read a value -
                      the legend and the table below carry every number. */}
                  <title>{`${slice.value}: ${slice[valueKey]}`}</title>
                </circle>
              )
            })}
          </g>
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-3xl font-bold leading-none text-slate-900">{total}</span>
      ...
      {/* The legend is not optional here: a slice's identity is otherwise
          ...
          two equal slices are still distinguishable. */}
      <ul className="w-full min-w-0 space-y-1.5">
        {slices.map((slice) => (
          <li key={slice.value} className="flex items-center gap-2.5 text-sm">
            <span
            ...
            <span className="shrink-0 tabular-nums font-semibold text-slate-900">{slice[valueKey]}</span>
            <span className="w-10 shrink-0 text-right tabular-nums text-xs text-slate-400">
              {Math.round((slice[valueKey] / total) * 100)}%
            </span>
          </li>
        ))}
      </ul>
