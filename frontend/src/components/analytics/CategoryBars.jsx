...
// How much of the list stands above the fold: five bars and a sliver of the
// sixth, the sliver being what says there is a sixth at all.
//
// 40px a bar (20px of name, 4px of gap, the 8px track, 4px of padding each
// side) with 4px between them, 23px for the sticky header, and 16px of the
// sixth showing: 5*40 + 4*4 + 23 + 16 = 255px.
const LIST_MAX = '15.9375rem'
 ...
 *
 * Colour comes from the canvas-wide assignment, so a category is the same
 * colour in both views and switching between them doesn't repaint anything.
 * Past the palette's six slots the bars go grey, the same as the ring's tail
 * and the treemap's - but every value still gets its own bar, because a bar is
 * labelled with its own name and grey costs nothing where the name is beside
 * the mark.
 *
 * Five bars stand above the fold and the rest are a scroll away. This is the
 * ranked view, so that is the top five and the tail in the order a reader
 * wants them - and the card stays the height of the three beside it on the
 * canvas however many values the dimension turns out to have.
 *
...
export function CategoryBars({
  ...
  const rows = rankedWithColor(items, valueKey)
  ...
  return (
    ...
    <div className="mx-auto w-full min-w-0 max-w-184" onMouseLeave={() => setHovered(null)}>
      {/* The header scrolls with the list rather than sitting above it, and
          sticks to the top once it gets there. Outside the scroller it would
          be a column heading that no longer lines up with its column: a list
          long enough to scroll takes a scrollbar's width out of the rows
          underneath and nothing out of the header. */}
      <div className="overflow-y-auto overscroll-contain pr-0.5" style={{ maxHeight: LIST_MAX }}>
        {/* Padding rather than a margin under it: a margin is transparent, and
            the rows would scroll through the gap between the stuck header and
            the first of them. */}
        <div className="sticky top-0 z-10 flex items-center justify-end gap-4 bg-white pb-2 pr-6 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
